...
%
% Copyright (C) 2014 Marta M. Betcke
...
if cs_transform.coarseVisible
    % visible filter in coarse scale in frequency domain
    cs_transform.Psi = @(x) vectCurvelet(visibleCoarseScale(fdct_wrapping(reshape(x,cs_transform.imageSize),...
    cs_transform.real, cs_transform.finest, cs_transform.nscales, cs_transform.nangles_coarse),cs_transform.theta,...
    cs_transform.setting),cs_transform.maskC,cs_transform.vcSize);
else
    % Curvelet transform: vectorized image -> vectorized Curvelet
    % coefficients without visible filter in coarse scale
...
% Function for displaying Curvelet coefficients as an image (takes vectorized Curvelet coefficients)
cs_transform.imagePsi = @(x_psi) real(fdct_wrapping_dispcoef(unvectCurvelet(x_psi, cs_transform.S, cs_transform.maskC),cs_transform.nscales));  
