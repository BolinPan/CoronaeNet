function x = computeCurveletImage(C, is_real, M, N, sCoeff, mode)
%
% COMPUTECURVELETIMAGE recovers the an Curvelet coefficient in image domain
%
% Inputs
%   C       - Cell array containing single curvelet coefficient (see
%             description in fdct_wrapping.m), i.e. specifying a curvelet
%             coefficient in a given scale and angle while other
...
%
% Outputs
%   x       -    curvelet coefficient in image domain
%
...
% recover in image domain
if sCoeff == 1 % Coarsest wavelet level
    switch mode
        case 'all'
            X_all = fftshift(fft2(ifftshift(squeeze(C{1}{1}))))/sqrt(prod(size(squeeze(C{1}{1})))).* lowpass;          
        case 'invisible'
            %M1 = M1/2;
            %M2 = M2/2;
            Xj_vis = fftshift(fft2(ifftshift(squeeze(C{1}{1}))))/sqrt(prod(size(squeeze(C{1}{1}))));
            Xj_inv = fftshift(fft2(ifftshift(squeeze(C{1}{2}))))/sqrt(prod(size(squeeze(C{1}{2}))));
            X_vis = Xj_vis .* lowpass;
            X_inv = Xj_inv .* lowpass;
            X_all = X_vis + X_inv;
    end
    X = X_all;
    %loc_1 = Xj_topleft_1 + (0:(2*floor(4*M1)));
...
elseif (X_size{sCoeff}(1) < M) % higher scales of size smaller than original image
    X = X(X_index{sCoeff}(1,:) ,X_index{sCoeff}(2,:));
else % other scales of size larger than original image needs to wrap back
    % Finest level
    M1 = N1/3;
    M2 = N2/3;
    if finest == 1,
